...
def load_all():
    ...
    stats.setdefault('processed_star_payments', [])
...
async def show_u_chrg(event):
    ...
    await event.edit(
        f"💰 **شحن النقاط بالنجوم**\n\n"
        f"💳 رصيدك الحالي: `{user_bal['points']} نقطة`\n\n"
        f"اختر الباقة التي تريد شراءها:\n"
        f"⭐ كل 1 نجمة = 1 نقطة",
        parse_mode='markdown',
        buttons=[
            [
                Button.inline("⭐ 10 نجمة = 5 نقطة", "charge_stars:10"),
                Button.inline("⭐ 50 نجمة = 30 نقطة", "charge_stars:50")
            ],
            [
                Button.inline("⭐ 100 نجمة = 60 نقطة", "charge_stars:100"),
                Button.inline("⭐ 500 نجمة = 270 نقطة", "charge_stars:500")
            ],
            [
                Button.inline("⭐ 1000 نجمة = 650 نقطة", "charge_stars:1000")
            ],
            [
                Button.inline('🔙 العودة للقائمة الرئيسية', 'user_main_menu')
            ]
...
async def hndl_cb(event):
    ...
    if is_adm(uid):
        ...
        if data == 'user_main_menu': 
            ...
            try:
                points = int(data.split(':', 1)[1])
            except (ValueError, IndexError):
                await event.answer("❌ باقة غير صالحة.", alert=True)
                return

            allowed_packages = {10, 50, 100, 500, 1000}
            if points not in allowed_packages:
                await event.answer("❌ باقة غير صالحة.", alert=True)
                return

            uid = str(event.sender_id)
            payload = f"charge_points:{uid}:{points}"

            try:
                bot.send_invoice(
                    chat_id=int(uid),
                    title="💰 شحن رصيد البوت",
                    description=f"إضافة {points} نقطة إلى رصيدك داخل البوت.",
                    invoice_payload=payload,
                    provider_token="",
                    currency="XTR",
                    prices=[LabeledPrice(f"{points} نقطة", points)]
                )
                await event.answer("✅ تم إرسال فاتورة الدفع.")
            except Exception as e:
                print(f"Stars invoice error: {e}")
                await event.answer("❌ تعذر إنشاء فاتورة الدفع.", alert=True)
...
def hndl_pre_cq(pre_cq):
    try:
        payload = pre_cq.invoice_payload or ""
        parts = payload.split(":")

        if len(parts) != 3 or parts[0] != "charge_points":
        ...
        if str(pre_cq.from_user.id) != uid:
            bot.answer_pre_checkout_query(
                pre_cq.id,
                ok=False,
                error_message="❌ هذه الفاتورة ليست لحسابك."
            )
            return

        if pre_cq.currency != "XTR" or pre_cq.total_amount != points or points <= 0:
            bot.answer_pre_checkout_query(
                pre_cq.id,
                ok=False,
                error_message="❌ بيانات الدفع غير صحيحة."
            )
            return
        ...
        bot.answer_pre_checkout_query(
            pre_cq.id,
            ok=False,
            error_message="❌ حدث خطأ أثناء التحقق من الدفع."
        )
...
def hndl_paid(paid_msg):
    try:
        payment = paid_msg.successful_payment
        ...
        # تأكيد أن الدفع لهذا المستخدم وأن المبلغ والعملة صحيحان.
        if str(paid_msg.from_user.id) != uid:
            print("Rejected Stars payment: user mismatch")
            return

        if payment.currency != "XTR" or payment.total_amount != points or points <= 0:
            print("Rejected Stars payment: amount/currency mismatch")
            return

        # منع إضافة الرصيد مرتين إذا وصل تحديث الدفع أكثر من مرة.
        processed = stats.setdefault('processed_star_payments', [])
        if charge_id in processed:
            return

        user_bal = get_syyad_bal(uid)
        user_bal['points'] += points

        stats['total_revenue_stars'] = stats.get('total_revenue_stars', 0) + points
        processed.append(charge_id)

        # الاحتفاظ بآخر 500 عملية دفع فقط.
        ...
        try:
            admin_id = int(syyad_conf['admin_ids'][0])
            bot.send_message(
                admin_id,
                f"💰 **دفعة Stars جديدة**\n\n"
                f"👤 المستخدم: `{uid}`\n"
                f"⭐ المبلغ: `{points}` نجمة\n"
                f"🧾 Charge ID: `{charge_id}`",
                parse_mode='Markdown'
            )
        except Exception as admin_error:
            print(f"Admin payment notification error: {admin_error}")
